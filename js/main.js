...
document.title='💩 DUNGEON v2.2.2';$('#load-code').placeholder='POOPRPG220-... / POOPRPG213-...';
let saveHeading=$('#save-status').closest('section')?.querySelector('.font-semibold');if(saveHeading?.nextElementSibling)saveHeading.nextElementSibling.textContent='v2.1.1〜v2.2.2のコードを移行できます。自動セーブ対応。最大8時間の放置報酬。';
...
$('#load-btn').onclick=()=>{try{let c=$('#load-code').value.trim(),prefix=['POOPRPG220-','POOPRPG213-','POOPRPG212-','POOPRPG211-'].find(p=>c.startsWith(p));if(!prefix)throw Error('対応コードではありません');let env=JSON.parse(dec(c.slice(prefix.length)));if(!env||typeof env.body!=='string'||hash(env.body)!==env.sig)throw Error('コード破損');let p=JSON.parse(env.body),candidate=normalize(p.g),elapsed=Math.max(0,(Date.now()-num(p.t,Date.now()))/1000);candidate.paused=false;candidate.deadUntil=0;candidate.enemy=null;G=candidate;ensureV22State();G.hp=stats().hp;spawn();offlineRewards(elapsed);saveLocal();$('#save-status').textContent=prefix==='POOPRPG220-'?'ロード成功。':'旧バージョンからv2.2.2へ移行してロード成功。';touch()}catch(e){$('#save-status').textContent='ロード失敗：'+e.message}};
