...
document.title='💩 DUNGEON v2.2.1';$('#load-code').placeholder='POOPRPG220-... / POOPRPG213-...';
let saveHeading=$('#save-status').closest('section')?.querySelector('.font-semibold');if(saveHeading?.nextElementSibling)saveHeading.nextElementSibling.textContent='v2.1.1〜v2.2.1のコードを移行できます。自動セーブ対応。最大8時間の放置報酬。';
...
$('#load-btn').onclick=()=>{try{let c=$('#load-code').value.trim(),prefix=['POOPRPG220-','POOPRPG213-','POOPRPG212-','POOPRPG211-'].find(p=>c.startsWith(p));if(!prefix)throw Error('対応コードではありません');let env=JSON.parse(dec(c.slice(prefix.length)));if(!env||typeof env.body!=='string'||hash(env.body)!==env.sig)throw Error('コード破損');let p=JSON.parse(env.body),candidate=normalize(p.g),elapsed=Math.max(0,(Date.now()-num(p.t,Date.now()))/1000);candidate.paused=false;candidate.deadUntil=0;candidate.enemy=null;G=candidate;ensureV22State();G.hp=stats().hp;spawn();offlineRewards(elapsed);saveLocal();$('#save-status').textContent=prefix==='POOPRPG220-'?'ロード成功。':'旧バージョンからv2.2.1へ移行してロード成功。';touch()}catch(e){$('#save-status').textContent='ロード失敗：'+e.message}};
...
function loop(now){let raw=Math.min(.25,(now-last)/1000||0);last=now;if(!G.paused){if(G.deadUntil<=Date.now()){if(!G.enemy)spawn();let dt=raw*G.simSpeed,s=stats();G.hp=Math.min(s.hp,G.hp+s.hp*s.regen*dt);if(G.enemy){ht+=dt;et+=dt;let guard=0;while(G.enemy&&ht>=1/s.sp&&guard++<20){ht-=1/s.sp;heroHit();s=stats()}guard=0;while(G.enemy&&et>=((G.enemy.boss?1.5:G.enemy.elite?1.85:2.35)/G.enemy.enemySpeed)&&guard++<20){let iv=(G.enemy.boss?1.5:G.enemy.elite?1.85:2.35)/G.enemy.enemySpeed;et-=iv;enemyHit();if(G.deadUntil>Date.now())break}}}}uiT+=raw;saveT+=raw;if(uiT>.16){uiT=0;if(G.deadUntil&&G.deadUntil<=Date.now()&&!G.enemy){G.hp=stats().hp;spawn();log('💩 再出撃。')}renderFast();if(staticDirty)renderStatic()}if(saveT>=5){saveT=0;saveLocal()}requestAnimationFrame(loop)}
