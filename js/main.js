...
const LOCAL_KEY='POOP_DUNGEON_V230',LEGACY_KEYS=['POOP_DUNGEON_V224','POOP_DUNGEON_V223','POOP_DUNGEON_V220','POOP_DUNGEON_V213'];
document.title='💩 DUNGEON v2.3.0';$('#load-code').placeholder='POOPRPG230-... / POOPRPG224-...';
let saveHeading=$('#save-status').closest('section')?.querySelector('.font-semibold');if(saveHeading?.nextElementSibling)saveHeading.nextElementSibling.textContent='v2.1.1〜v2.3.0のコードを移行できます。自動セーブ対応。最大8時間の放置報酬。';
...
function payloadNow(){return{v:230,t:Date.now(),g:snapshotState()}}
...
$('#save-btn').onclick=()=>{let body=JSON.stringify(payloadNow());$('#save-code').value='POOPRPG230-'+enc(JSON.stringify({body,sig:hash(body)}));$('#save-status').textContent='保存コード生成完了。自動セーブも有効です。'};
$('#load-btn').onclick=()=>{try{let c=$('#load-code').value.trim(),prefix=['POOPRPG230-','POOPRPG224-','POOPRPG223-','POOPRPG220-','POOPRPG213-','POOPRPG212-','POOPRPG211-'].find(p=>c.startsWith(p));if(!prefix)throw Error('対応コードではありません');let env=JSON.parse(dec(c.slice(prefix.length)));if(!env||typeof env.body!=='string'||hash(env.body)!==env.sig)throw Error('コード破損');let p=JSON.parse(env.body),candidate=normalize(p.g),elapsed=Math.max(0,(Date.now()-num(p.t,Date.now()))/1000);candidate.paused=false;candidate.deadUntil=0;candidate.enemy=null;G=candidate;ensureV22State();G.hp=stats().hp;spawn();offlineRewards(elapsed);saveLocal();$('#save-status').textContent=prefix==='POOPRPG230-'?'ロード成功。':'旧バージョンからv2.3.0へ移行してロード成功。';touch()}catch(e){$('#save-status').textContent='ロード失敗：'+e.message}};
